    ...
    public void afterPropertiesSet() {
        ...
        threadPoolExecutor.execute(() -> {
            while (!stop) {
                ...
                List<ObjectRecord<String, HashMap>> objectRecords = redisTemplate.opsForStream()
                        .read(HashMap.class, streamReadOptions, StreamOffset.create(Cosntants.STREAM_KEY_SINGLE, ReadOffset.from(readOffset.toString())));
                if (CollectionUtils.isEmpty(objectRecords)) {
                    log.warn("没有获取到数据");
                    continue;
                }

                for (ObjectRecord<String, HashMap> objectRecord : objectRecords) {
